...
5. **写场景**：在目标环境目录 `scenarios/<env>/` 新建 `*.yaml`（`<env>` = 要跑的 `--env` 名；可复制 `scenarios/stable/example.yaml`），用 `${state.*}` 复用铺底、固定前缀造新数据、写 `assert`。
6. **验证**：`pnpm start run --env <env> scenarios/<env>/<file>.yaml`，断言全绿。失败按 response 摘要分流修正（鉴权/前缀/业务码/路径）。
